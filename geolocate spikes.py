# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 13:02:12 2024
...
pf = pd.read_excel('C:/Users/archi/OneDrive - University of Bath/Industry Project/Bath VLF 2024/Bath potential fireballs.xlsx')
pf = pf.set_index('num')

#fb_spike = pd.read_excel('C:/Users/archi/OneDrive - University of Bath/Industry Project/Bath VLF 2024/fireball search.xlsx')
fb_spike = pd.read_excel('C:/Users/archi/OneDrive - University of Bath/Industry Project/Bath VLF 2024/fireball_spikes.xlsx')
...
for i,j in fb_spike.iterrows():
    ...
    try:
        ...
        for node in node_config.keys():
            ...
            # File path to data files (if have hard drive then: /Volumes/WD_ITP/Bath_VLF_2023_decoded)
            folder_path = 'D:/bath_students_2023/bath_students_2023/'+str(n)+'/'
            # Create list of file paths for each file in the folder
        ...
        geo_lat.append(f"{result['x'][1]:.2f}")
        geo_long.append(f"{result['x'][0]:.2f}")
...
fb_spike['geo_lat'] = geo_lat
fb_spike['geo_long'] = geo_long
'''       
...
# Calculate longitude and latitude errors (assuming 50 km error)
error_km = 50
error_degrees = error_km / 111.32  # Approximate conversion from km to degrees

# Add error bars for d_lat13 and d_long13
ax.errorbar(d_long13, d_lat13, xerr=error_degrees, yerr=error_degrees, fmt='none', ecolor='r', elinewidth=1)
ax.errorbar(d_long, d_lat, xerr=error_degrees, yerr=error_degrees, fmt='none', ecolor='b', elinewidth=1)
