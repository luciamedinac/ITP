# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 12:29:45 2024
...
for n in pf.index:
    ...
    savgol_waveform = np.abs(datadict['569218Q0B0025002A']['waveform'][l1])
    '''
    ...
    plt.show()
    '''
    pf.loc[n,'outsidefb_noise_mag'] = np.mean(savgol_waveform)
    ...
    savgol_waveform = np.abs(datadict['569218Q0B0025002A']['waveform'][l2])
    '''
    ...
    pf['diffnoise'] = (pf['duringfb_noise_mag'] - pf['outsidefb_noise_mag'])*2*10**-5
    

plt.plot(pf['abs_mag'],pf['diffnoise'],'b.')
plt.xlabel('Absolute Magnitude')
plt.ylabel('Percentage Difference in noise')
plt.title('Difference in Noise for Fireball Magnitude')
plt.legend()
plt.show()
